...
async function processHtmlForImages(
  htmlContent: string,
  exportDir: string,
  baseFileName: string,
...
async function processHtmlContentForPdf(htmlContent: string) {
...
<body>
    ...
    const tempHtmlPath = path.join(os.tmpdir(), `temp_export_${Date.now()}.html`);
    await fsPromises.writeFile(tempHtmlPath, finalContent);
    ...
    const win = new BrowserWindow({
      ...
      webPreferences: {
        nodeIntegration: false,
        contextIsolation: true,
        sandbox: false, // Allow file access
...
ipcMain.handle('drinkReminder:toggle', async () => {
  try {
    ...
    const config = drinkReminderService.getConfig()
    const nextTime = drinkReminderService.getNextReminderTime()
    return { success: true, data: { ...config, enabled, nextReminderTime: nextTime } }
  } catch (error) {
    return { success: false, error: error instanceof Error ? error.message : String(error) }
  }
})
